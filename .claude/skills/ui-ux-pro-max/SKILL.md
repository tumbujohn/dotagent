...
- Designing new pages (landing pages, dashboards, admin panels, SaaS, mobile apps)
- Creating or refactoring UI components (buttons, modals, forms, tables, charts, etc.)
- Choosing color schemes, typography systems, spacing rules, or layout systems
...
- Work that only involves API or database design
- Performance optimization unrelated to the interface
- Infrastructure or DevOps work
- Non-visual scripts or automation tasks

**Rule of thumb**: use this Skill if the task changes how something **looks, feels, moves, or is interacted with**.

## Rule Categories by Priority

*For human/AI reference: follow priority 1→10 to decide which category to focus on first; use `--domain <Domain>` to query full details when needed. The script does not read this table.*
...
- Default to the **Phosphor (`@phosphor-icons/react`)** icon library. The entries listed in `src/ui-ux-pro-max/data/icons.csv` are only common recommended icons, not the full set.
- When no suitable icon is found in the recommendation table:
  - **Prefer continuing to pick any more semantically fitting icon from Phosphor's full icon set**;
  - If Phosphor has no ideal option either, fall back to **Heroicons (`@heroicons/react`)**, keeping the style consistent (linear/filled, stroke weight, corner style).
